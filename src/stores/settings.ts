 ...
 *
 * Sync + restore state lives in `useSyncStore` (S3); `disconnect` calls
 * `useSyncStore().reset()` so a stale modal can't act on cleared credentials.
 */
export const useSettingsStore = defineStore('settings', () => {
   ...
   * Resets sync store FIRST so any stashed pendingRestore / pendingConflict
   * can't try to apply against cleared credentials.
   */
